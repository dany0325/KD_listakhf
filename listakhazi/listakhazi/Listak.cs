...
namespace Listak
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Innen kezdje a megoldásokat!

            //2)
            Console.WriteLine(AtlagSzamitas(szamsorA));

            //4)
            List<int> ujSzamsor = ListakOsszeFuzese(szamsorA, szamsorB);

            //5)
            int canonSzam = 0;

            foreach (string s in tintapatronok)
            {
                if (s.Contains("Canon"))
                {
                    canonSzam++;
            ...
            //8)
            Console.WriteLine(LegtobbJel(jelek));
        }


        static float AtlagSzamitas(List<int> szamsor)
        {
            float osszeg = 0;
            float db = szamsor.Count;
            foreach (float f in szamsor)
            {
                osszeg += f;
            }
            return osszeg / db;
        ...
        static void Rendezes(List<int> szamsor)
        {
            for (int i = 0; i < szamsor.Count; i++)
            {
                for (int j = i + 1; j < szamsor.Count; j++)
                {
                    if (szamsor[i] > szamsor[j])
                    {
                        int temp = szamsor[i];
                        szamsor[i] = szamsor[j];
                        szamsor[j] = temp;
        ...
        static List<int> ListakOsszeFuzese(List<int> szamsorA, List<int> szamsorB)
        {
            List<int> rendezett = new List<int>();

            rendezett.AddRange(szamsorA);
            rendezett.AddRange(szamsorB);
        ...
        static int MaxDeRossz(List<int> szamsor)
        {
            int legnagyobb = 0;

            foreach (int i in szamsor)
            {
                if (i > legnagyobb)
                {
                    legnagyobb = i;
        ...
        static char LegtobbJel(List<char> jelek)
        {
            List<char> jelChars = new();
            List<int> jelSzamlalo = new();

            foreach (char c in jelek)
            {
                if (!jelChars.Contains(c))
                {
                    jelChars.Add(c);
                    jelSzamlalo.Add(1);
                }
                ...
                if (jelChars.Contains(c))
                {
                    int index = jelChars.IndexOf(c);
                    jelSzamlalo[index]++;
                }
            }

            return jelChars[jelSzamlalo.IndexOf(MaxDeRossz(jelSzamlalo))];
        }
    }
}
